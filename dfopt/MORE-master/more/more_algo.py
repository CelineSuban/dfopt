...
class MORE:
    ...
    def get_beta(self, old_dist):
        if self.gamma > 0:
            # beta = self.gamma * old_dist.entropy + (1 - self.gamma) * self.h_0
            beta = self.gamma * (old_dist.entropy - self.h_0) + self.h_0
        else: # maybe set beta to 0 to use the else case, print beta! see how it decreases
    ...
    def step(self, old_dist, surrogate):
        ...
        for i in range(10):
            ...
            # this is done not to update eta_0
            if success:
                break
